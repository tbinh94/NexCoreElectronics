const fs = require('fs');
const path = 'd:\\Study\\Project\\NextGenEcommerce\\backend\\src\\data\\products.json';
...
try {
    ...
    if (startIndex !== -1) {
        ...
        for (let i = startIndex; i < lines.length; i++) {
            if (lines[i].includes('"highlights": [')) {
                highlightsStart = i;
                break;
            }
        }
        ...
        if (highlightsStart !== -1) {
            let endIndex = -1;
            for (let i = highlightsStart; i < lines.length; i++) {
                if (lines[i].includes('"id": 283')) {
                    endIndex = i - 2;
                    break;
                }
            }
            ...
            if (endIndex !== -1) {
                ...
                missingProducts.forEach(p => {
                    ...
                    newLines.push(indented + ',');
                });

                lines.splice(highlightsStart, endIndex - highlightsStart + 1, ...newLines);
            ...
            console.log("Highlights start not found");
        }
    } else {
        console.log("ID 253 not found");
    }
} catch (e) {
    console.error(e);
}
